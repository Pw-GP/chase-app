'use client'
import { RegisterItem, STATUSES, CLOSED_STATUSES } from '@/types'
import { StatusTag, TypePill, IconClose, IconSend, IconCopy, IconCheck, fmtDate, showToast } from './ui'
import { isOverdue, updateItem } from '@/lib/store'
import { useState } from 'react'

interface Props { item: RegisterItem; onClose: () => void; onUpdated: () => void }

export default function DetailDrawer({ item, onClose, onUpdated }: Props) {
  const [status, setStatus]     = useState(item.status)
  const [notes, setNotes]       = useState(item.notes)
  const [followUp, setFollowUp] = useState(item.followUp || item.suggestedFollowUp || '')
  const [genLoading, setGenLoading] = useState(false)
  const ov     = isOverdue(item)
  const closed = CLOSED_STATUSES.includes(status)
  const statuses = STATUSES[item.type] ?? []

  function save() {
    updateItem(item.id, { status, notes })
    showToast('Saved')
    onUpdated()
  }
  ...
  function markClosed() {
    ...
    const ns = (map[item.type] ?? 'Closed') as typeof status
    updateItem(item.id, { status: ns, notes })
    setStatus(ns)
    showToast('Marked ' + ns)
    onUpdated()
  }
  ...
  async function generateFollowUp() {
    ...
    try {
      ...
      const text = data.suggested_follow_up_email || 'Could not generate. Check API key in Settings.'
      setFollowUp(text)
      updateItem(item.id, { followUp: text })
      ...
      <div className="detail-panel">
        ...
        <div className="detail-scroll">
          <div>
            ...
            <div className="detail-chips">
              <StatusTag status={status} overdue={ov} />
              {item.discipline && <span style={{ fontSize:10.5, color:'var(--text3)' }}>{item.discipline}</span>}
            </div>
          </div>
          <div className="detail-block">
            ...
            <div className="detail-row"><span className="detail-label">Due date</span><span className={`detail-value ${ov ? 'due-over' : ''}`}>{fmtDate(item.dueDate)}</span></div>
          </div>
          {item.actionRequired && (
            <div className="detail-block">
              ...
              <div className="detail-text">{item.actionRequired}</div>
            </div>
          )}
          {item.emailText && (
            <details className="detail-block">
              <summary style={{ padding:'9px 12px', fontSize:10, fontWeight:600, letterSpacing:'.08em', textTransform:'uppercase', color:'var(--text3)', display:'flex', alignItems:'center', justifyContent:'space-between', listStyle:'none', background:'var(--bg)', cursor:'pointer' }}>
                Source email <span style={{ fontSize:9, opacity:.6 }}>▼ expand</span>
              </summary>
              <div className="detail-email">{item.emailText}</div>
            </details>
          )}
          <div className="detail-block">
          ...
          {!closed && (
            <div>
              <button className="btn" onClick={generateFollowUp} disabled={genLoading} style={{ fontSize:10.5, display:'flex', alignItems:'center', gap:5 }}>
                {genLoading ? <span className="spinner" style={{ width:13, height:13 }} /> : <IconSend />}
                Generate follow-up email
              </button>
              {followUp && (
                <div style={{ marginTop:10 }}>
                  <div className="follow-up-box">{followUp}</div>
                  <button className="btn" style={{ marginTop:7, fontSize:10.5, display:'flex', alignItems:'center', gap:5 }} onClick={() => { navigator.clipboard.writeText(followUp); showToast('Copied') }}>
                    <IconCopy /> Copy
                  </button>
                </div>
              )}
        ...
        <div className="detail-actions">
          ...
          <div style={{ display:'flex', gap:7 }}>
            ...
            {!closed && (
              <button className="btn" style={{ fontSize:10.5, display:'flex', alignItems:'center', gap:5 }} onClick={markClosed}>
                <IconCheck /> Close
              </button>
            )}
          </div>
