...
function TiltCard({ children, className, style }: { children: React.ReactNode, className?: string, style?: React.CSSProperties }) {
  ...
  function onMove(e: React.MouseEvent) {
    ...
    el.style.boxShadow = `${-x * 10}px ${-y * 10}px 28px rgba(42,79,168,0.10), 0 4px 16px rgba(41,52,58,0.08)`
  }
  function onLeave() {
    const el = ref.current; if (!el) return
...
function CustomCursor() {
  ...
  useEffect(() => {
    function move(e: MouseEvent) { pos.current = { x: e.clientX, y: e.clientY } }
  ...
  return (
    <>
      ...
      <div ref={trailRef} style={{
        ...
        border:'1.5px solid rgba(42,79,168,0.35)',
        transform:'translate(-50%,-50%)',
        background:'rgba(97,192,191,0.06)',
      }}/>
    </>
  )
}
  ...
  return (
    <div className="landing" style={{ cursor:'none' }}>
      ...
      <style>{`
        ...
        .how-card-anim-a { animation: floatA 4s ease-in-out infinite; }
        .how-card-anim-b { animation: floatB 5s ease-in-out infinite 0.8s; }
        .how-card-anim-c { animation: floatC 4.5s ease-in-out infinite 1.4s; }
        ...
        .role-pill { transition: background 0.18s, border-color 0.18s, color 0.18s; }
        ...
        .pricing-card:hover { transform: translateY(-4px); box-shadow: 0 8px 28px rgba(42,79,168,0.10); }
        .mock-kpi { transition: transform 0.15s, box-shadow 0.15s; }
        .mock-kpi:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(41,52,58,0.10); }
      `}</style>
      ...
      <div className="landing-hero">
        <div className="landing-eyebrow hero-fadein">Construction communication, simplified</div>
        <h1 className="landing-h1 hero-fadein-2">Every action. Every follow-up. <em>Nothing missed.</em></h1>
        <p className="landing-sub hero-fadein-3">Paste a project email and Chase automatically extracts actions, approvals, RFIs and follow-ups into one clean dashboard.</p>
        <div className="landing-actions hero-fadein-3">
        ...
        <div className="landing-note hero-fadein-3">No credit card &nbsp;·&nbsp; 5-minute setup &nbsp;·&nbsp; Works with any email client</div>
      </div>
      ...
      <div className="how-section">
        ...
        <div className="how-grid">
          {HOW_STEPS.map((step, i) => (
            <TiltCard key={step.n} className={`how-card how-card-anim-${['a','b','c'][i]}`}>
